...
- Generate prompt text with direct vendors such as Gemini, MiMo, or DeepSeek, or with OpenRouter, then push the result into either the image or voice workflow
...
- Apply generated output directly to:
  - the image prompt
  - the voice prompt
- Pick direct-vendor models from configured model lists, or use `openrouter/free` / any live OpenRouter model slug for aggregator-backed text generation
...
GEMINI_KEY=your_gemini_text_key_here
GEMINI_API_KEY=your_gemini_image_key_here
...
- `GEMINI_KEY`
  Required for the Gemini prompt writer. Direct text vendors use the convention `{vendor slug without underscores}_KEY`, uppercased.
- `GEMINI_API_KEY`
  Required for Gemini image generation.
- `MIMO_KEY`
  Required for the Xiaomi MiMo prompt writer.
- `DEEPSEEK_KEY`
  Required for the DeepSeek prompt writer. DeepSeek is called with thinking mode explicitly disabled.
- `OPENROUTER_API_KEY`
...
Runs a one-shot direct-vendor text request for Gemini, MiMo, DeepSeek, or future direct vendors.

### `GET /api/direct-llm/models?vendor=...`

Returns model options for a direct vendor. Providers with a model-list endpoint are queried first; documented defaults are used as fallback.
...
- Sends a one-shot request with the selected direct-vendor model
...
- Direct vendors and OpenRouter are prompt-writing utilities, not a conversation layer.
